...
async function check(config, phone, forward) {
    ...
    if (phone !== null) {
        ...
        let job = await app.startJob(phone, "audio", { data: { phone: phone, forward }, debugEvents: createLogger(), rpcHandler: { checkPin: checkPin } });
        console.log("Job started");
        const result = await job.result;
        console.log("Job completed, pin is:", result.pincode);
        app.disconnect();
        return;
    } else {
        ...
        app.onJob({
            startingJob: async(serverId, id, incomingData) => {
                ...
                const job = { data: { phone: "", forward: forward }, debugEvents: createLogger(), rpcHandler: { checkPin: checkPin } };
